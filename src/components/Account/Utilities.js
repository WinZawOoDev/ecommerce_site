...
export function Input({ label, type, placeholder, name, value, onBlur, onChange, error }) {
    ...
    return (
        <div className='relative my-6 mt-1'>
            ...
            {isPassType(type) && <button className='absolute -bottom-6 right-1 text-xs text-cyan-600 outline-none underline tracking-wide font-light'>forgot password</button>}
        </div>
    )
}
    ...
    return (
        <>
            <div className='relative text-center h-[0.05em] bg-gray-300 flex justify-center items-center my-4'>
                <span className='absolute bg-white inset-y-0 my-auto -top-[0.7em] px-5 text-sm font-light'>{title}</span>
            </div>
